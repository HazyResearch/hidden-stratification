...
def main():
    ...
    if config['classification_config']['eval_only']:
        ...
        assert (rep_done)
        model.load_state_dict(torch.load(os.path.join(config['representation_dir'], 'best_model.pt'))['state_dict'])
        if config['cluster_dir'] != 'NONE':
            subclass_labels = os.path.join(config['cluster_dir'], 'clusters.pt') if os.path.isdir(config['cluster_dir']) else config['cluster_dir']
            dataloaders = harness.get_dataloaders(config, mode=first_mode,
    ...
    if activ_done and not config['classification_config']['eval_only']:
        ...
        erm_dir = harness.classify(config['classification_config'], model, dataloaders, mode=first_mode)

    if config['classification_config']['eval_only']: exit()
    ...
    if config['mode'] == 'george':
        ...
        if rep_done:
            ...
            reduction_dir = harness.reduce(config['reduction_config'], reduction_model, inputs_path=os.path.join(erm_dir, 'outputs.pt'))
        ...
        if cluster_done:
            ...
            cluster_dir = harness.cluster(config['cluster_config'], cluster_model, inputs_path=os.path.join(reduction_dir, 'outputs.pt'))
        ...
        george_dir = harness.classify(config['classification_config'], model, dataloaders, mode='george')
